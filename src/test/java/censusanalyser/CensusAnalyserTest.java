...
public class CensusAnalyserTest {
    ...
    @Test
    public void givenIPL2019BattingRecords_WhenSortedOnRunsWithBestBattingAverage_ShouldReturnSortedResult(){
        CensusAnalyser censusAnalyser = new CensusAnalyser();
        try {
            censusAnalyser.loadIPL2019BattingData(IPL2019_MOSTRUNS_FILE_PATH);
            String sortedRunsData = censusAnalyser.getRunsSortedData();
            IPL2019BattingRecordCSV[] battingRecordCSV = new Gson().fromJson(sortedRunsData,IPL2019BattingRecordCSV[].class);
            ArrayList<IPL2019BattingRecordCSV> battingRecordFiltered = new ArrayList<>();
            for(IPL2019BattingRecordCSV r: battingRecordCSV){
                if(r.average>30){
                    battingRecordFiltered.add(r);
